...
const challengeId = ref(sessionStorage.getItem('challengeId') || '');
const email = ref(sessionStorage.getItem('email') || '');
const method = ref(sessionStorage.getItem('method') || '');
const code = ref('');
const msg = ref('');
...
async function onSubmit(e: Event) {
  e.preventDefault();
  msg.value = '';
  if (!challengeId.value || !code.value) {
    msg.value = 'Faltan datos';
    return;
  }
  busy.value = true;
  try {
    ...
    msg.value = err?.response?.data?.message || err?.message || 'Error al verificar código';
  } finally {
    busy.value = false;
  }
}
...
<template>
  <div class="verify-container">
    ...
    <div class="verify-card">
      ...
      <p class="subtitle">
        Se envió un código por <strong>{{ method || 'email/SMS' }}</strong> a <strong>{{ email }}</strong>.
      </p>
      ...
      <form @submit="onSubmit" novalidate class="verify-form">
        ...
        <p v-if="msg" class="message" :class="{ success: msg.includes('token') || msg.includes('dashboard') }">
          {{ msg }}
        </p>
      </form>
